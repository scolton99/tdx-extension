// Modified from https://github.com/liufeifeiholy/vite-plugin-zip
...
const zip = (options = {}) => {
  ...
  return {
    ...
    closeBundle() {
      ...
      dir = dir || outputPath;
      outputName =
        outputName || `${new Date().getMonth() + 1}${new Date().getDate()}`;
...
const generateZip = (arr, dir, outputName) => {
  ...
  zipfile.outputStream
    ...
    .on("close", function () {
      console.log(`\n${c.green(`✓ Extension archive created`)}`);

      console.log(
        `\n${c.bold(c.inverse(c.blue(" PATH ")))}${c.blue(
          ` ${c.white.bold(outFile)}\n`
        )}`
      );
    });
  zipfile.end();
